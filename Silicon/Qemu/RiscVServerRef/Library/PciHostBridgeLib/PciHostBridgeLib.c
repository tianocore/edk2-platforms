...
STATIC PCI_ROOT_BRIDGE  mRootBridge = {
  ...
  /* PCI_ROOT_BRIDGE_APERTURE Bus; Bus aperture which can be used by the
     root bridge. */
  {
    /* auto filled in by the library implementation */
    0,
    0
  },
  ...
  )
{
  ...
  //
  // All output parameters in this function are initialized to zero.
  // If the DT node does not describe a given resource type, the
  // corresponding output values will remain zero. The caller can use this
  // to determine which resource types are present.
  //
  *IoBase       = 0;
  *IoSize       = 0;
  *Mmio32Base   = 0;
  ...
  *BusMax = SwapBytes32 (((CONST UINT32 *)Prop)[1]);
  ...
  if (EFI_ERROR (Status)) {
    if (Status == EFI_ACCESS_DENIED) {
      DEBUG ((
        DEBUG_VERBOSE,
        "%a: ECAM space [0x%Lx+0x%Lx) already mapped in GCD\n",
        __func__,
        ConfigBase,
        ConfigSize
        ));
  ...
  if (*IoSize != 0) {
    ...
    if (EFI_ERROR (Status) && (Status == EFI_ACCESS_DENIED)) {
      DEBUG ((
        DEBUG_VERBOSE,
        "%a: IO space [0x%Lx+0x%Lx) already mapped in GCD\n",
        __func__,
        *IoBase + IoTranslation,
        *IoSize
        ));
      return EFI_SUCCESS;
    }
  ...
  )
{
  ...
  if (IoSize != 0) {
    ...
    mRootBridge.Io.Base  = MAX_UINT64;
    mRootBridge.Io.Limit = 0;
  }
