...
#define NOR_BLOCK_SIZE  SIZE_256KB
  ...
  )
{
                                     ...
                                     ))
  {
    ...
    while (PropSize >= (4 * sizeof (UINT32)) && Num < MAX_FLASH_BANKS) {
      ...
      //
      if ((PcdGet64 (PcdRiscVFdBaseAddress64) + PcdGet32 (PcdRiscVFirmwareFdSize) > Base) &&
          ((Base + Size) > PcdGet64 (PcdRiscVFdBaseAddress64)))
      {
        continue;
      }
      ...
      mNorFlashDevices[Num].BlockSize         = NOR_BLOCK_SIZE;
      Num++;
    }
