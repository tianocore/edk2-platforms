...
//
// Retrieve the SMEM base address and size from the platform-defined FixedPCDs.
//
UINT64 SmemBase = FixedPcdGet64(PcdSmemBaseAddress);
UINT32 SmemSize = FixedPcdGet32(PcdSmemSize);
...
{
  if ((UINT64)Offset + (UINT64)Size > SmemSize) {
  ...
  )
{
  ...
  return (VOID *)(UINTN)(SmemBase + (UINT64)Offset);
}
