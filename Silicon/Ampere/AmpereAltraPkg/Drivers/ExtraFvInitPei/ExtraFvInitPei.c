...
{
  UINT32                      UefiExtraFdSize        = PcdGet32 (PcdUefiExtraFdSize);
  UINT64                      UefiExtraFdBaseAddress = PcdGet64 (PcdUefiExtraFdBaseAddress);
  ...
  if (Fv == NULL) {
    DEBUG ((DEBUG_ERROR, "%a Fail to allocate memory for Fv\n", __func__));
