...
## How to build
- GCC_AARCH64_PREFIX=aarch64-linux-gnu- build -b DEBUG -a AARCH64 -t GCC -p edk2-qualcomm/KodiakMinPlatformPkg/KodiakMinPlatformPkg.dsc -j $WORKSPACE/Build/KodiakMinPlatformPkg.log
...
```bash
cd Platform/Qualcomm/KodiakFamily/KodiakMinPlatformPkg
./BuildKodiakMinPlatformPkg.sh [OPTIONS]
```
...
```bash
# Default DEBUG build
./BuildKodiakMinPlatformPkg.sh

# RELEASE build with 8 threads
./BuildKodiakMinPlatformPkg.sh -b RELEASE -n 8

# Build both DEBUG and RELEASE
./BuildKodiakMinPlatformPkg.sh -b DEBUG,RELEASE -n 4
```
...
```
KODIAKMINPLATFORMPKG.fd
    │  objcopy  (binary → AArch64 ELF object)
    │  ld       (link at load address 0x9FC00000)
    ▼
...
```
Build/KodiakMinPlatformPkg/<BUILD>_<TOOLCHAIN>/FV/uefi.elf
```
