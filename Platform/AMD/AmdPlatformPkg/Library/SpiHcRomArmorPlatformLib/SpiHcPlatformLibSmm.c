...
/**
  ...
  @param[in] Protocol   Pointer to the protocol GUID that triggered the notification.
  @param[in] Interface  Pointer to the protocol interface.
  @param[in] Handle     Handle on which the protocol was installed.

  @retval EFI_SUCCESS       Lock-down completed successfully.
  @retval EFI_DEVICE_ERROR  PSP mailbox SPI mode entry failed.
**/
EFI_STATUS
EFIAPI
AmdSpiHcEventNotify (
  ...
  )
{
  ...
  if (PcdGet8 (PcdAmdPspRomArmorSelection)) {
    // Call PSP MailBox to change to PSP SPI mode
    Status = gMmst->MmLocateProtocol (
