...
/**
  ...
  @retval EFI_SUCCESS  Device path returned successfully.
**/
EFI_STATUS
EFIAPI
GetSpiHcDevicePath (
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (!EFI_ERROR (Status)) {
      if (!mPspMailboxSpiMode) {
        // Enable UseSpi100
        MmioOr8 (
          (mHcAddress & 0xFFFFFF00) + FCH_SPI_MMIO_REG20,
          BIT0
          );
        // Set the Value for NormSpeed and FastSpeed
        InternalClockValue = (InternalClockValue << 12 | InternalClockValue << 8) & MAX_UINT16;
      } else {
  ...
  )
{
  ...
  if (  (This->MaximumTransferBytes > (MAX_UINT32 - 6))
     || (WriteBytes > This->MaximumTransferBytes + 6)
