...
/**
  Check that SPI Controller is not busy.

  @retval EFI_SUCCESS       SPI controller is idle and ready.
  @retval EFI_DEVICE_ERROR  SPI controller is busy or unreachable.
**/
EFI_STATUS
EFIAPI
  ...
  )
{
  ...
  if (mPspMailboxSpiMode || (mHcAddress > 0xFFFFFF00)) {
...
/**
  Check for SPI transaction failures.

  @retval EFI_SUCCESS       SPI transaction completed without error.
  @retval EFI_ACCESS_DENIED SPI FIFO pointer error bit is set.
  @retval others            SPI controller busy or unreachable.
**/
EFI_STATUS
EFIAPI
...
/**
  If SPI controller is not busy, execute SPI command, then wait for completion.

  @retval EFI_SUCCESS  SPI command executed and completed successfully.
  @retval others       SPI controller busy, busy on return, or transaction error.
**/
EFI_STATUS
EFIAPI
FchSpiExecute (
...
/**
  ...
  Calls from SMM are only valid during SMM; restore state will wipe changes.

  @param[in] HcAddress  MMIO base address of the SPI host controller.
  @param[in] Opcode     SPI opcode value to block.

  @retval EFI_SUCCESS           Opcode blocked successfully.
  @retval EFI_INVALID_PARAMETER HcAddress is out of range.
  @retval EFI_OUT_OF_RESOURCES  No free RestrictedCmd slot is available.
**/
EFI_STATUS
EFIAPI
...
/**
  ...
  @param[in] HcAddress  MMIO base address of the SPI host controller.
  @param[in] Opcode     SPI opcode value to unblock.
