...
#  for Secure Boot.
#
# Copyright (c) 2024, Rebecca Cran <rebecca@bsdio.com>. All rights reserved.<BR>
...
if [ -z "${USE_EXISTING_SB_KEYS}" ]; then
    ...
    if [ ! -f "openssl.cnf" ]; then
      cp -vf "${WORKSPACE}/edk2-platforms/Platform/Ampere/Tools/openssl.cnf" .
    fi
    ...
    openssl req -config openssl.cnf -new -x509 -newkey rsa:2048 -subj "/CN=${BOARD_NAME} Update Key/" -keyout keys/update_key.priv -outform DER -out certs/update_key.cer -days 7300 -nodes -sha256
