...
usage () {
  ...
  echo "  -s, --silent                     Make use of silent mode of (n)make."
  echo ""
...
OPTIONS=$(${GETOPT_COMMAND} -o t:b:fFshl:m:p: --long toolchain:,build:,linuxboot:,manufacturer:,platform:,flash,full-flash,silent,help -- "$@")
eval set -- "$OPTIONS"
