...
EDKII_PLATFORM_REPOSITORY_INFO VExpressPlatRepositoryInfo = {
  ...
  */
  ...
  {
    { REFERENCE_TOKEN (CacheInfo[0]) }  // -> cluster 1's L2 cache
  },
  // Resources private to each individual cluster 0 core instance
  {
    { REFERENCE_TOKEN (CacheInfo[1]) }, // -> cluster 0 core's L1 I-cache
    { REFERENCE_TOKEN (CacheInfo[2]) }  // -> cluster 0 core's L1 D-cache
  },
  // Resources private to cluster 1 (shared among cores)
  {
    { REFERENCE_TOKEN (CacheInfo[3]) }  // -> cluster 2's L2 cache
  },
  // Resources private to each individual cluster 1 core instance
  {
    { REFERENCE_TOKEN (CacheInfo[4]) }, // -> cluster 1 core's L1 I-cache
    { REFERENCE_TOKEN (CacheInfo[5]) }  // -> cluster 1 core's L1 D-cache
  },
