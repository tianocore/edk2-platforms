 ...
 */
#define IFD_F_STATUS_STAGING  (1 << 0)   /**< Image File is on staging */

#define IFD_IS_OPENED(Ifd)   ((((Ifd)->ImageFile != NULL)))
#define IFD_ON_STAGING(Ifd)  ((BOOLEAN) ((((Ifd)->Status) & IFD_F_STATUS_STAGING) != 0))
  ...
  for (Idx = 0; Idx < Count; Idx++) {
    ...
    if (IFD_IS_OPENED (Ifd)) {
      if (Mode == CLEAR_CANCEL) {
         ...
         */
        ASSERT (Ifd->OpType != FWU_STREAM_WRITE);
  ...
  )
{
  ...
  if ((ReqData->OperationType != FWU_STREAM_READ) &&
      (ReqData->OperationType != FWU_STREAM_WRITE))
  {
    return PSA_MM_FWU_NOT_AVAILABLE;
  }
  ...
  if (ReqData->OperationType == FWU_STREAM_WRITE) {
    if ((Ifd->ClientPermissions & FWU_WRITE_PERMISSION) == 0) {
      return PSA_MM_FWU_NOT_AVAILABLE;
    }

    if (!IFD_ON_STAGING (Ifd)) {
      return PSA_MM_FWU_DENIED;
    }
  }
  ...
  )
{
  ...
  if (Ifd->OpType != FWU_STREAM_WRITE) {
  ...
  )
{
  ...
  if (ReqData->AcceptanceReq == 0) {
    if ((Ifd->ClientPermissions & FWU_ACCEPT_AFTER_ACTIVATION) &&
        ((Ifd->OpType == FWU_STREAM_WRITE) || !IsCorrectBoot))
    {
      return PSA_MM_FWU_DENIED;
    }

    if ((Ifd->OpType == FWU_STREAM_WRITE) && (Ifd->Pos == 0)) {
      return PSA_MM_FWU_DENIED;
    }
  }
  ...
  if (ReqData->AcceptanceReq == 0) {
     ...
     */
    if ((Ifd->OpType == FWU_STREAM_READ) && (mFwState != FW_STORE_STAGING)) {
  ...
  )
{
  ...
  if (MmHandlerContext->CommProtocol == CommProtocolFfa) {
    ...
    if (FfaMsgInfo->ServiceType == ServiceTypeMmCommunication) {
      ...
      // via ff-a direct request message v2.
