  ...
  )
{
  ...
  if (!StrCmp (PartitionEntry->PartitionName, PartitionName)) {
    return TRUE;
  }
...
/**
  ...
  @param [in]   OpType                 FWU_STREAM_READ / FWU_STREAM_WRITE
  ...
  )
{
  ...
  FwsMetadata = FwsDeviceData->FwsMetadata;
  BankIdx     = (OpType == FWU_STREAM_WRITE) ? FwsDeviceData->UpdateIndex : FwsDeviceData->BootIndex;
