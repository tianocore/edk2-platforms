...
{
  /**
   * Arm doesn't update firmware via Coalescing and firmware storage is
   ...
   * If TRUE is returned, FmpDevicePkg locks the Variable Storage and the
   * firmware storage during the EndOfDxe phase.
   * This means firmware update when using fwupd's capsule on disk method,
   * CapsuleApp or runtime firmware cannot update then ESRT information
   * saved in variable storage properly as they are updated after EndOfDxe phase.
   *
