...
{
  /**
   * Arm doesn't update firmware via Coalescing because firmware storage is
   * managed in Secure world.
   * That means we don't need to lock the device and firmware update can be
   * progressed after EndOfDxe phase.
   * If returns TRUE, FmpDevicePkg locks the Variable ESRT related on EndofDxe
   * and the firmware storage on specific phase (default: EndOfDxe phase).
   * Firmware update using fwupd's capsule on disk method, CapsuleApp or
   * runtime firmware cannot update ESRT information saved in variable storage
   * properly as they happen after EnfOfDxe phase.
   *
   * So, to generate ESRT table and update firmware properly, return FALSE.
   */
  return FALSE;
}
