  ...
  )
{
  ...
  * VersionString = (CHAR16*)AllocateZeroPool(PcdGetSize(PcdSystemFirmwareFmpVersionString));
  StrCpyS(*VersionString, PcdGetSize(PcdSystemFirmwareFmpVersionString), PcdGetPtr(PcdSystemFirmwareFmpVersionString));
...
{
  DEBUG ((DEBUG_INFO, "UPX: FmpDeviceLock() for system FLASH\n"));
  // TODO: Add lock logic
  return EFI_UNSUPPORTED;
}
