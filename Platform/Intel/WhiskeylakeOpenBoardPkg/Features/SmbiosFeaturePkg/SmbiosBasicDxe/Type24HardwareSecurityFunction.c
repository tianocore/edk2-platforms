...
{
    EFI_STATUS                  Status;
    SMBIOS_TABLE_TYPE24         *PcdSmbiosRecord;
    SMBIOS_TABLE_TYPE24         *SmbiosRecord;
    EFI_SMBIOS_HANDLE           SmbiosHandle;
    UINTN                       TableSize;

    PcdSmbiosRecord = PcdGetPtr (PcdSmbiosType24HardwareSecurity);

    //
    // Create table size based on string lengths
    //
    TableSize = sizeof (SMBIOS_TABLE_TYPE24) + 1 + 1;
    ...
    //
    // Fill in type 24 fields
    //
    SmbiosRecord->Hdr.Type = SMBIOS_TYPE_HARDWARE_SECURITY;
