  ...
  )
{
  ...
  for (VariableMtrrIndex = 0; VariableMtrrIndex < VariableMtrrCount; VariableMtrrIndex++) {
    if (VariableMtrr[VariableMtrrIndex].Valid) {
      DEBUG (
             (DEBUG_INFO, "CACHE - 0x%016lx 0x%016lx %d\n",
  ...
  if (IsAmdCpu) {
    ...
    if ((MsrSysCfg & (BIT21 | BIT22)) == (BIT21 | BIT22)) {
      ...
      for (VariableMtrrIndex = 0; VariableMtrrIndex < VariableMtrrCount; VariableMtrrIndex++) {
        ...
        // WB
        if (VariableMtrr[VariableMtrrIndex].Type == MTRR_CACHE_WRITE_BACK) {
          continue;
        }
        ...
        if (VariableMtrr[VariableMtrrIndex].BaseAddress > BaseAddr) {
          DEBUG (
                 (DEBUG_INFO, "CACHE(TOM2) - 0x%016lx 0x%016lx %d\n",
                  BaseAddr,
                  VariableMtrr[VariableMtrrIndex].BaseAddress - BaseAddr,
  ...
  )
{
  ...
  for (Index = 0; Index < VariableMtrrCount; Index++) {
    DEBUG (
           (DEBUG_INFO, "Variable MTRR[%02d]: Base=%016lx Mask=%016lx\n",
            Index,
