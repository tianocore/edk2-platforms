...
NextCpu:
  ...
  if (EFI_ERROR (Status)) {
    ...
    if (!EFI_ERROR (Status) && (PropSize == sizeof (UINT32))) {
      ...
      GuestIndexBits = FdtReadU32Be (Prop);

      if (GuestIndexBits >= 32) {
        Topo->AiaGuests = MAX_UINT32;
      } else {
        Topo->AiaGuests = (GuestIndexBits == 0) ?
                          0U :
                          ((1U << GuestIndexBits) - 1U);
      }
    } else {
      Topo->AiaGuests = 0;
    }
  ...
  if (EFI_ERROR (Status)) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      DEBUG ((DEBUG_ERROR, "%a: Failed to read UART reg\n", __func__));
      return EFI_PROTOCOL_ERROR;
    }
  ...
  if (EFI_ERROR (Status)) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      DEBUG ((DEBUG_ERROR, "%a: Failed to read IOMMU reg\n", __func__));
      return EFI_PROTOCOL_ERROR;
    }
